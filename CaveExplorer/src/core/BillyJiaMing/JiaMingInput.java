...
public class JiaMingInput {
	...
	public static void promptAttack() {
	...
	public static boolean isValidCheatCode() {
		if(input.equals("out")) {
	...
	private static void pickDirection(int shipLength) {
		...
		if(EventBillyJiaMing.inGame) {
			updateUserMap(shipLength,dir);
		}
	}

	private static void updateUserMap(int shipLength, int dir) {
		if(dir==UP) {
	...
	private static void pickACoord(int shipLength) {
		...
		while(!validShipCoord(shipLength)) {
			printUserMap();
	...
	private static boolean checkAdjacentTiles(int shipLength) {
	...
	private static boolean validShipCoord(int shipLength) {
		input=CaveExplorer.in.nextLine();
		if(isValidCheatCode()) {
			return isValidCheatCode();
		}
		...
		if(commaIndex>leftParaIndex && rightParaIndex>commaIndex) {
			...
			if(isNumber(row) && isNumber(col)) {
				...
				if(JiaMingInput.row>=0 && JiaMingInput.row<=9 && JiaMingInput.col>=0 && JiaMingInput.col<=9) {
					return checkAdjacentTiles(shipLength);
				}
			}
		}
		return false;
	...
	private static boolean validAttackCoord() {
		input=CaveExplorer.in.nextLine();
		if(isValidCheatCode()) {
			return isValidCheatCode();
		}
